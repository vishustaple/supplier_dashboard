            ...
            <div class="sb-sidenav-menu">
                <div class="nav side_bar_admin">
                    ...
                    <div class="collapse {{ (isset($pageTitleCheck) && in_array($pageTitleCheck, ['Account Validation Rebate Report', 'Business Report', 'Quarter Report', 'Consolidated Supplier Report', 'Supplier Rebate Report', 'Validation Rebate Report', 'Commission Report'])) ? 'show' : '' }}" id="submenuSupplier">
                        ...
                        @endif
                        @if(in_array('Account Validation Rebate Report', auth()->user()->permissions->pluck('name')->toArray()) || auth()->user()->user_type != \App\Models\User::USER_TYPE_USER)
                            <a class="nav-link ml-3 {{ (isset($pageTitleCheck) && $pageTitleCheck == 'Account Validation Rebate Report') ? 'active' : '' }}" href="{{route('report.type', ['reportType' => 'account_validation_rebate_report'])}}">Account Validation Rebate Report</a>
                        @endif
                    ...
                    @if(in_array('SQL Maintenance', auth()->user()->permissions->pluck('name')->toArray()) || !in_array(auth()->user()->user_type, [\App\Models\User::USER_TYPE_USER, \App\Models\User::USER_TYPE_ADMIN]) && !auth()->user()->needs_password_change)
                    ...
                    @endif
                    @if(in_array('Ask Your Database', auth()->user()->permissions->pluck('name')->toArray()) || !in_array(auth()->user()->user_type, [\App\Models\User::USER_TYPE_USER]) && !auth()->user()->needs_password_change)
