        ...
        <div class="container">
            @if (isset($orders) && !empty($orders))
                    ...
                    <div class="row_details">
                        ...
                        <p><b>Customer Name</b>: {{ $orders->customer_name??' ' }}</p>
                        <p><b>Supplier Name</b>: {{ $orders->supplier_name }}</p>
                        <p><b>Amount</b>: {{ '$'.$orders->amount }}</p>
                        <p><b>Date</b>: {{ date_format(date_create($orders->date), 'm/d/Y') }}</p>
                        <p><b>Internal Reporting Name</b>: {{ $orders->internal_reporting_name ?? ' ' }}</p>
                        <p><b>QBR</b>: {{ $orders->qbr ??' ' }}</p>
                        <p><b>Supplier Acc Rep</b>: {{ $orders->supplier_acct_rep??' ' }}</p>
                        <p><b>Mangement Fee</b>: {{ $orders->management_fee??' '  }}</p>
                        <p><b>Record type </b>: {{ $orders->record_type ??' ' }}</p>
                        <p><b>CPG Sales Representative </b>: {{ $orders->cpg_sales_representative??' '  }}</p>
                        <p><b>CPG Customer Service Rep </b>: {{ $orders->cpg_customer_service_rep??' '  }}</p>
                        <p><b>SF Cat</b>: {{ $orders->sf_cat??' '  }}</p>
                        <p><b>Rebate Freq</b>: {{ $orders->rebate_freq	??' '  }}</p>
                        <p><b>Member Rebate</b>: {{ $orders->member_rebate??' '  }}</p>
                        <p><b>Comm Rate</b>: {{ $orders->comm_rate??' '  }}</p>
                    </div>
            ...
            @if (isset($account) && !empty($account))
                    <div class="row_details">
                        ...
                        <p><b>Temp End Date</b>: {{ $account->temp_end_date }}</p>
                        ...
                        <p><b>Temp Active Date</b>: {{ $account->temp_active_date }}</p>
            ...
            @if (isset($catalog) && !empty($catalog))
                @foreach ($catalog as $order)
                    <div class="row_details">
                        ...
                        <p><b>Supplier Acc Rep</b>: {{ $order->supplier_acct_rep??' ' }}</p>
                        <p><b>Mangement Fee</b>: {{ $order->management_fee??' '  }}</p>
