...
class CategorySupplier extends Model
{
    ...
    static function supplierValidationShowDataTable($filter=[]) {
        $supplierColumnArray = [
            'suppliers.supplier_name',
            'suppliers.category',
        ];
        ...
        $query = self::select([
            ...
            'suppliers.category as category',
            'suppliers.supplier_name as supplier_name',
            'rebate_supplier_fields.id as manage_columns_id',
        ])
        ->leftJoin('rebate_supplier_fields', function($join) {
            $join->on('rebate_supplier_fields.supplier_id', '=', 'suppliers.id')
                 ->where('rebate_supplier_fields.deleted', '=', 0);
        });
