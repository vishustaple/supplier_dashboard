...
class OfficeDepotDataAdd extends Command
{
    ...
    public function handle()
    {
        ...
        //                 ->selectRaw(
        //                     'sku, uom, qty_shipped, product_description,
        ...
        $records = DB::table('staples_order')->select('id', 'shipped_date_id')->get();
        // dd($records);
        foreach ($records as $key => $value) {
            if (!empty($value->shipped_date_id)) {
                DB::table('staples_order')->where('id', $value->id)->update(['shipped_date_id' => Carbon::createFromTimestamp(ExcelDate::excelToTimestamp($value->shipped_date_id))->format('Y-m-d H:i:s')]);
            }
        }
        // 
        // dd($newFinalArray);
